...
Export a repo's MoTeC M1 `.m1dbc` CAN databases as standard Vector `.dbc`
files — and tell CI when a committed export has gone stale.

A firmware repo keeps its CAN databases as MoTeC `.m1dbc` files, which nothing
outside M1-Build can read. `m1-dbc` renders each one as a standard Vector `.dbc`
...
[m1-project](https://github.com/nedlane/m1-project) — this verb used to live in
`m1-project` as `export-dbc`, and moved out because it shares none of that
tool's `--project` model.

## Install

Prebuilt binaries for Linux, macOS, and Windows are attached to each
[release](https://github.com/C-Nucifora/m1-dbc/releases). Or build from source:

```sh
cargo install --git https://github.com/C-Nucifora/m1-dbc.git --tag <latest>
```

## Usage

```sh
m1-dbc export           # regenerate the .dbc exports
m1-dbc export --check   # CI/hook: are they up to date? (writes nothing)
```

`export` is a subcommand rather than a bare flag so the binary can grow a second
verb later without breaking hooks that already call `m1-dbc export --check`.

## Configuration

`m1-dbc` takes no path argument. It is configured by the `[dbc]` section of the
...
**Adopting `m1-dbc` in a repo whose `.dbc` files were generated by something
else therefore needs one regeneration commit**: run `m1-dbc export` once, commit
